...
# Tmux or xterm 
# Fluxion must be executed inside an existing tmux session if enabled
TMuxEnable=1
...
while [ "$1" != "" ] && [ "$1" != "--" ]; do
  case "$1" in
    ...
    -m|--multiplexer) declare FLUXIONTMux=1;;
...
# Handle xterm window for airodump-ng
fluxion_target_get_candidates_xterm_window() {
...
fluxion_target_get_candidates() {
  ...
  echo -e "$FLUXIONVLine Got what you need? Press c to continue."
  ...
  if [[ "$TMuxEnable" -eq 0 ]]; then
    ...
    tmux split-window -p 25 "airodump-ng -Mat WPA "${2:+"--channel $2"}" "${3:+"--band $3"}" -w \"$FLUXIONWorkspacePath/dump\" $1";
    tmux select-pane -U;
    while [ true ]; do 
      (sleep 1)
      read -n1 -s -r key
